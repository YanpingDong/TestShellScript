...
# parse jtl file in turn
ls -1 $jtl_dir | while read LINE
do
	jtl_file="$jtl_dir/$LINE"
	# get the jtl_file total line number
	...
	# Error Number
	awk -F ',' 'BEGIN{sum=0} {if($4!=200 && $4!=201) sum+=1} END {print "Error Number : ", sum}' $jtl_file >> $result_file
	
	# Error%
	awk -F ',' 'BEGIN{sum=0} {if($4!=200 && $4!=201) sum+=1} END {print "Error% : ", sum/NR*100,"%"}' $jtl_file >> $result_file
	...
	rm -f $tmp_file
	echo "Throughput (/s): $throughput" >> $result_file
	
	# Time per user requests
	time_per_user_req=`awk -F ',' 'BEGIN{sum=0} {sum+=$2} END {print sum/NR}' $jtl_file`
	echo "Time per user requests (ms): $time_per_user_req" >> $result_file
	...
	rm -f $tmp_file
	echo "Median (ms): $median_num" >> $result_file
	...
	# discard digits after the decimal point. the follow: 9.1-->9 1-->9
	...
	rm -f $tmp_file
	echo "90% Line (ms): $ninety_line_num" >> $result_file
	
	# Min
	min_req_time=`awk -F ',' '{print $2}' $jtl_file | sort -n | head -n 1`
	echo "Min (ms): $min_req_time" >> $result_file
	
	# Max
	max_req_time=`awk -F ',' '{print $2}' $jtl_file | sort -n | tail -n 1`
	echo "Max (ms): $max_req_time" >> $result_file
	...
	echo "Time per requests (accross all concurrency) ms: $time_per_concurrency_req" >> $result_file
